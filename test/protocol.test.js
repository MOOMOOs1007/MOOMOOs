...
test('soop4j compatible join packet',()=>{const p=parsePacket(joinPacket('12345'));assert.equal(p.code,2);assert.equal(p.parts[1],'12345');assert.equal(p.parts.length,7)});
