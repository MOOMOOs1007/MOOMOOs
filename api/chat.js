...
async function discover(streamer){
 ...
 return {domain,port,chatNo,streamerName:String(channel.BJNICK||streamer).trim().slice(0,60)};
}
export default async function handler(req,res){
 ...
 try{
  ...
  const {domain,port,chatNo,streamerName}=await discover(streamer);if(finished)return;
  ...
  socket.on('message',data=>{const p=parsePacket(data);if(!p)return;
   ...
   if(p.code===2&&!entered){entered=true;clearTimeout(deadline);send(res,{type:'entered',streamer,streamerName,duration:30,message:`${streamerName} 방송 · 채팅 수집 시작 · 30초`});collectionTimer=setTimeout(()=>finish('done','30초 채팅 수집 완료'),30000);return;}
