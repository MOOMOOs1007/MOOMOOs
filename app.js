...
function render(){
 ...
 $('round').textContent='ROUND '+String(state.round).padStart(2,'0');$('streamerName').textContent=state.streamerName?state.streamerName+' 방송 연결됨':'방송 연결 대기';$('progress').textContent=done+' / 5 완성';$('count').textContent=pool.length;$('status').textContent=state.status;$('collectState').textContent=state.checking?'확인 중':state.connecting?'채팅 연결 중':state.collecting?'수집 중':state.rolling?'추첨 중':state.streamerName?'연결됨':'대기';
 $('connect').disabled=busy||isOverlay;$('connect').textContent=state.checking?'확인 중…':state.streamerName?'✓ 연결됨':'방송 연결';$('connect').classList.toggle('connected',!!state.streamerName&&!busy);$('collect').disabled=busy||isOverlay||!state.streamerName;$('collect').textContent=state.connecting?'채팅 연결 중…':state.collecting?'30초 남음':pool.length?'다시 수집':'채팅 수집';$('draw').disabled=busy||isOverlay||!pool.length||!!result;$('reroll').disabled=busy||isOverlay||!pool.length||!result;$('next').disabled=busy||isOverlay||done===5;$('clear').disabled=busy||isOverlay;$('demo').disabled=busy||isOverlay;
...
$('connect').onclick=connectBroadcast;$('collect').onclick=startCollection;$('streamer').addEventListener('input',()=>{if(state.streamerName){state.streamerName='';state.connectedStreamer='';state.status='주소가 바뀌었어요. 다시 방송에 연결해 주세요.';render()}});$('draw').onclick=spin;$('reroll').onclick=spin;$('next').onclick=()=>{const n=state.results.findIndex((r,i)=>!r&&i>state.active);state.active=n<0?state.results.findIndex(r=>!r):n;render()};$('resetAll').onclick=()=>reset(true);$('clear').onclick=()=>{state.pools[state.active].clear();state.results[state.active]=null;render()};$('closeModal').onclick=closeModal;$('modalBackdrop').onclick=closeModal;
