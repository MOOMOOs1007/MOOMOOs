...
function snapshot(){return {fields,active:state.active,round:state.round,results:state.results,pools:state.pools.map(pool=>[...pool.values()]),enabled:state.enabled,manualAnswers:state.manualAnswers,manualConfirmed:state.manualConfirmed,checking:state.checking,connecting:state.connecting,collecting:state.collecting,until:state.until,rolling:state.rolling,spin:state.spin,filterPrefix:state.filterPrefix,streamerName:state.streamerName,connectedStreamer:state.connectedStreamer,status:state.status}}
...
function applySnapshot(data){if(!isOverlay)return;try{const next=typeof data==='string'?JSON.parse(data):data;if(!next||!Array.isArray(next.pools))return;if(Array.isArray(next.fields))fields=next.fields;Object.assign(state,next,{pools:next.pools.map(list=>new Map(list.map(item=>[item.id,item])))});if(!Array.isArray(state.manualConfirmed))state.manualConfirmed=Array(fields.length).fill(false);render()}catch{}}
...
function closeModal(){if(state.connecting||state.collecting||state.rolling)return;modalOpen=false;$('drawModal').classList.remove('open');$('drawModal').setAttribute('aria-hidden','true');document.body.style.overflow='';if(!isOverlay){state.spin=null;publish()}}
...
async function startCollection(){
 ...
 const token=++generation;controller=new AbortController();state.pools[state.active].clear();state.results[state.active]=null;const manual=state.manualAnswers[state.active].trim();if(manual&&state.manualConfirmed[state.active])state.pools[state.active].set(`manual-${state.active}`,{id:`manual-${state.active}`,name:'직접 추가',text:manual});state.connecting=true;state.status=state.filterPrefix?`'${state.filterPrefix}'로 시작하는 채팅을 수집할 준비 중…`:'전체 채팅을 수집할 준비 중…';render();
 try{
...
function syncOverlaySpin(){
 if(!isOverlay)return;const spin=state.spin;
...
function render(){
 ...
 $('closeModal').disabled=busy;if(modalOpen&&!state.rolling&&!$('winner').classList.contains('show'))$('modalStatus').textContent=state.status;tick();publish();syncOverlaySpin();
}
...
$('connect').onclick=connectBroadcast;$('collect').onclick=startCollection;document.querySelectorAll('.prefix-filter').forEach(input=>input.addEventListener('change',()=>{state.filterPrefix=input.checked?input.value:'';state.status=state.filterPrefix?`'${state.filterPrefix}'로 시작하는 채팅만 수집합니다.`:'전체 채팅을 수집합니다.';render()}));$('streamer').addEventListener('input',()=>{if(state.streamerName){state.streamerName='';state.connectedStreamer='';state.status='주소가 바뀌었어요. 다시 방송에 연결해 주세요.';render()}});$('draw').onclick=spin;$('modalReroll').onclick=spin;$('confirmWinner').onclick=confirmResult;$('next').onclick=()=>{const candidates=state.results.map((r,i)=>({r,i})).filter(x=>state.enabled[x.i]&&!x.r);const after=candidates.find(x=>x.i>state.active);if(candidates.length)state.active=(after||candidates[0]).i;render()};$('resetAll').onclick=()=>reset(true);$('clear').onclick=()=>{state.pools[state.active].clear();state.results[state.active]=null;render()};$('closeModal').onclick=closeModal;$('modalBackdrop').onclick=closeModal;
...
$('demo').onclick=()=>{const i=state.active;state.pools[i].clear();const list=sample[i]||['행운 가득','평범하지만 행복','상상도 못한 결과'];list.forEach((text,j)=>state.pools[i].set('demo-'+i+'-'+j,{id:'demo-'+i+'-'+j,name:'테스트 '+(j+1),text}));const manual=state.manualAnswers[i].trim();if(manual&&state.manualConfirmed[i])state.pools[i].set('manual-'+i,{id:'manual-'+i,name:'직접 추가',text:manual});state.results[i]=null;state.status='테스트 후보로 룰렛을 시작했어요.';openModal('reroll');spin()};
